...
// The %p field width for printf() function is two characters per byte.
// For some environments, and two extra bytes for the leading "0x".
...
// Print a program counter, and its symol name
static void DumpPCAndSymbol(DebugWriter* writerfn, void* arg, void* pc,
  ...
  snprintf(buf, sizeof(buf), "%s@ %*p  %s\n",
           prefix, kPrintfPointerFieldWidth, pc, symol);
  writerfn(buf, arg);
}
#endif
...
DumpStackTraceAndExit() {
  ...
  // TODO(hamaji): Use a singal instead of sigaction
...
int UsecToCycles(int64 usec) {
  return usec;
}

WallTime WallTime_Now() {
  // Now, cycle clock is returning microseconds since the epoch.
...
const char* const basename(const char* filepath) {
...
// We use a atomic operation to prevent problems with calling CrashReason
